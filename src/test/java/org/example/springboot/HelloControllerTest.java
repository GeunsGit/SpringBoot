...
// 여기서는 Spring Runner라는 스프링 실행자를 사용한다. 즉, 스프링 부트 테스트와 JUnit 사이에 연결자 역할을 한다.
@WebMvcTest(controllers = HelloController.class,
excludeFilters = {
        @ComponentScan.Filter(type = FilterType.ASSIGNABLE_TYPE, classes = SecurityConfig.class)
})
// 여러 스프링 테스트 어노테이션 중, Web(Spring MVC)에 집중할 수 있는 어노테이션
// 선언할 경우 @Controller, @ControllerAdvice 등을 사용할 수 있음. 단, @Service, @Component, @Repository 등은 사용 불가
...
public class HelloControllerTest {
    ...
    public void hello가_리턴된다() throws Exception {
        ...
        mvc.perform(get("/hello"))  // MockMvc를 통해 /hello 주소로 HTTP GET 요청
                .andExpect(status().isOk()) // mvc.perform의 결과 검증, HTTP Header의 Status 검증, 여기선 200인지 아닌지 검증.
                .andExpect(content().string(hello));    // mvc.perform의 결과 검증, 응답 본문의 내용 검증.
            // Controlloer에서 "hello"를 리턴하기 때문에 이 값이 맞는지 검증.
    }
    ...
    public void helloDto가_리턴된다() throws Exception {
        ...
        mvc.perform(
                ...
                단, 값은 String만 허용함. 그래서 숫자/날짜 등의 데이터로 등록할 때는 문자열로 변경해야만 가능
                 */
